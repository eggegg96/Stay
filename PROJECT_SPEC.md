...
- 정산 트리거: 고객 체크아웃 완료 시점
- 정산 활성화: 사업자가 대시보드에서 직접 요청
- 정산 수단: 사업자 회원가입 시 등록한 계좌
- 정산 금액: 결제금액의 90% (플랫폼 수수료 10% 차감)
- 구현 수준: PG사 테스트 모드 연동 (실제 결제 API 호출, 로컬 환경 기준 동작)
...
- **2025.09.29**: 회원 기능 기초 구현 완료
- **2025.09.29**: 예약 시스템 방향성 확정
- **2025.10.04**: Phase 0 추가, 백엔드 DDD 구조 적용
- **2025.10.21**: 닉네임 기능 추가, OAuth HttpOnly Cookie 전환
- **2025.10.28**: Axios 전환, Zustand 전환, DB 테이블 섹션 제거
